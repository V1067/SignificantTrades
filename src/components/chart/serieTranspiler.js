...
export default class SerieTranspiler {
  constructor() {
    this.exchanges = socket.exchanges.map(a => a.id)
  }
  transpile(serie) {
    // input === delta = vbuy - vsell, cum(delta) + sma(ema(delta[1], 20), 10)
    let result = this.parse(serie.input)
  ...
  parseVariables(output, variables) {
    let variableMatch = null

    do {
      if ((variableMatch = VARIABLE_REGEX.exec(output, 'g'))) {
  ...
  parseFunctions(output, instructions) {
    ...
    do {
      if ((functionMatch = FUNCTION_REGEX.exec(output))) {
        ...
        const functionArgumentsNames = this.getParamNames(targetFunction).filter(a => a !== 'state')
  ...
  parseExchanges(output, exchanges) {
  ...
  test({ output, functions, variables, exchanges, references }, options) {
  ...
  getRenderer(previousRenderer, exchanges, references) {
    ...
    if (!previousRenderer) {
      renderer.timestamp = Math.floor(parseInt(+new Date() / 1000) / 10) * 10
    } else {
    ...
    for (const name of exchanges) {
      ...
      renderer.exchanges[name] = exchangeBar
    }
